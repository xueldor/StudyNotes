...
到`/var/lib/docker/containers/[容器ID]`），需要root账户，主要时hostconfig.json和config.v2.json两个文件。

注意，手动编辑这两个文件没有，不能起到容器已创建后修改、添加路径映射、端口映射的效果。

## 已有容器导出“docker run”命令

有时我们需要把一个容器删除，重新创建，但是不记得起初docker run的参数。作为业余用户不懂Docker Compose。

可以安装`runlike`：

```bash
# 安装runlike（Python 2/3均可）
pip install runlike
# 若提示权限不足，加--user
pip install --user runlike

# 基本用法：runlike 容器名/容器ID
runlike nginx-test  # 替换为你的容器名/ID
# 进阶：格式化输出（分行显示，更易读）
runlike -p nginx-test
```
...
当使用 --network host 时，容器与宿主机共享网络命名空间，容器内的 127.0.0.1 就直接指向宿主机。

```
# 容器内
curl http://127.0.0.1:1234  # 访问宿主机 1234 端口
```

默认桥接模式（不加 --network host）,容器内的 127.0.0.1（localhost）指向的是容器自己，而不是宿主机。

```
# 容器内访问 127.0.0.1:1234
# 实际访问的是容器内部的 1234 端口，不是宿主机的
curl http://127.0.0.1:1234  # 访问容器自己

curl http://172.17.0.1:7897  # 通过宿主机的IP访问宿主
```
...
这种方式是单向的。将外部流量导入容器，并不能让容器通过它来反向访问宿主机。
...
如果指定了-p, 外面访问不了。可能是因为：

1. 外面有防火墙。
2. 容器里没有监听端口
3. 容器内应该监听**0.0.0.0**而不是**127.0.0.1**
...
无输出：容器内 10000 端口无任何进程监听 → 端口转发自然失效（核心问题）；
